describe('Test Page for Cypress', () => {
    ...
    it("should display the correct heading text when the header is present or not", () => {
        /*
            This test is created to validate the behavior of the page heading in different scenarios.
            It focuses on two specific situations:

            1. When the header is present: It verifies that the displayed text in the header matches the expected
                value. Note that the test may occasionally fail due to randomly changing header text.
                (This is by design and can be found in the addEventListener() function in the index.html file.)

            2. When checking whether an element exists: It ensures that the page does not contain an h1 element, indicating
                the absence of a header. While this part of the test is not expected to fail, it is considered a flaky testing
                approach and should generally be avoided in favor of more reliable assertions.

            Note: Testing logic that relies on the presence or absence of an element at runtime can lead to flaky tests.
            Flaky tests produce inconsistent results and make it difficult to identify genuine issues.
            It's important to write tests with clear and deterministic conditions, relying on stable and
            predictable elements or properties for assertions.
        */
